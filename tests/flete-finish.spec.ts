...
test('Flete acabado: portada y acciones reales, sin ocultar desbordes', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('meta[name="flete-release"]')).toHaveAttribute('content', '0.5.0')
  ...
  await expect(page.locator('.service-choice[aria-pressed=true]')).toContainText('Pasajeros')
  await visibleContentFits(page)
...
test('Flete acabado: volver a un paso conserva el recorrido y el servicio', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Volver al paso 1: Recorrido', exact: true }).click()
  ...
  await expect(page.getByRole('heading', { name: '¿Cuántos viajan y cuándo?' })).toBeVisible()
  await page.reload()
  await expect(page.locator('.simple-route-summary')).toContainText('Punto de entrega de prueba')
...
test('Flete acabado: contenido largo, todas las pantallas pequeñas', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Continuar solicitud', exact: true }).click()
  await expect(page.locator('.simple-route-summary')).toContainText('Dirección de retiro')
