...
test('Flete: portada, recorrido rápido, FAQ, controles y CTA móvil', async ({ page }, testInfo) => {
  ...
  await page.locator('#quick-origin').fill('Origen de prueba, Aluminé')
  await page.locator('#quick-destination').fill('Destino de prueba, Aluminé')
  ...
  await expect(page.locator('.wizard-aside')).toContainText('Origen de prueba, Aluminé')
  ...
  await page.reload()
  await expect(page.getByRole('heading', { name: '¿Cuándo lo necesitás?' })).toBeVisible()
  await expect(page.locator('.wizard-aside')).toContainText('Destino de prueba, Aluminé')
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual((page.viewportSize()?.width ?? 0) + 1)
})
